...
func (l *DupeList) Search(key float64) *Node {
	l.RLock()
	defer l.RUnlock()
	x := l.header
	for i := l.level; i >= 0; i-- {
...
func (l *DupeList) SearchKeyVal(key float64, val []byte) *Node {
	l.RLock()
	defer l.RUnlock()
	x := l.header
	for i := l.level; i >= 0; i-- {
...
func (l *DupeList) Insert(key float64, val []byte) *Node {
...
func (l *DupeList) Delete(key float64) bool {
	update := make([]*Node, l.MaxLevel)
	x := l.header
	l.Lock()
	defer l.Unlock()
